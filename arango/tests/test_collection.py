...
def test_find_and_delete():
    assert collection.find_and_delete({'foo': 100}) == 0
    collection.insert_many([
        {'_key': '1', 'foo': 100},
        {'_key': '2', 'foo': 100},
        {'_key': '3', 'foo': 100},
    ...
    assert collection.find_and_delete({'foo': 200}) == 1
    assert '4' not in collection

    assert collection.find_and_delete({'foo': 300}, sync=True) == 1
    assert '4' not in collection

    assert collection.find_and_delete({'foo': 100}, limit=2) == 2
    count = 0
    for key in ['1', '2', '3']:
...
def test_find_between():
    ...
    result = collection.find_in_between(
        field='value',
        left=2,
        right=5,
        closed=True,
